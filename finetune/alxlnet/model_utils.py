...
def configure_tpu(FLAGS):
    ...
    if FLAGS.use_tpu:
        strategy = None
        @@#logging.info('Use TPU without distribute strategy.')
    elif FLAGS.num_core_per_host == 1:
        strategy = None
        @@#logging.info('Single device mode.')
    else:
...
def init_from_checkpoint(FLAGS, global_vars = False):
    ...
    if FLAGS.init_checkpoint is not None:
        ...
        @@#logging.info('Initialize from the ckpt {}'.format(init_checkpoint))
...
def get_train_op(FLAGS, total_loss, grads_and_vars = None):
    ...
    if FLAGS.warmup_steps > 0:
        warmup_lr = (
            tf.compat.v1.cast(global_step, tf.compat.v1.float32)
            / tf.compat.v1.cast(FLAGS.warmup_steps, tf.compat.v1.float32)
            * FLAGS.learning_rate
        )
    else:
        warmup_lr = 0.0
    ...
    clipped, gnorm = tf.compat.v1.clip_by_global_norm(gradients, FLAGS.clip)
...
def clean_ckpt(_):
    ...
    for (name, shape) in var_list:
        if not name.startswith('global_step') and 'adam' not in name.lower():
            ...
            @@#logging.info('Include {}'.format(name))
        else:
            @@#logging.info('Exclude {}'.format(name))

    @@#logging.info('Loading from {}'.format(input_ckpt))
    ...
    with @@#variable_scope(@@#get_variable_scope(), reuse = tf.compat.v1.AUTO_REUSE):
        tf_vars = [
    ...
    placeholders = [@@#placeholder(v.dtype, shape = v.shape) for v in tf_vars]
    ...
    if not tf.compat.v1.io.gfile.exists(output_model_dir):
        tf.compat.v1.io.gfile.mkdir(output_model_dir)
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint):
    ...
    for x in init_vars:
        (name, var) = (x[0], x[1])
        # @@#logging.info('original name: %s', name)
...
class AdamWeightDecayOptimizer(tf.compat.v1.train.Optimizer):
    ...
    def apply_gradients(self, grads_and_vars, global_step = None, name = None):
        ...
        for (grad, param) in grads_and_vars:
            ...
            next_m = tf.compat.v1.multiply(self.beta_1, m) + tf.compat.v1.multiply(
                1.0 - self.beta_1, grad
            )
            next_v = tf.compat.v1.multiply(self.beta_2, v) + tf.compat.v1.multiply(
                1.0 - self.beta_2, tf.compat.v1.square(grad)
            )
    ...
    def _do_use_weight_decay(self, param_name):
        ...
        if self.exclude_from_weight_decay:
            for r in self.exclude_from_weight_decay:
                if re.search(r, param_name) is not None:
                    @@#logging.info('Adam WD excludes {}'.format(param_name))
                    return False
        return True
