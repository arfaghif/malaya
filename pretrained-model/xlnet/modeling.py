...
def positionwise_ffn(inp, d_model, d_inner, dropout, kernel_initializer,
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        output = tf.compat.v1.layers.dropout(output, dropout, training=is_training,
                                   name='drop_2')
        output = tf.estimator.layers.layer_norm(output + inp, begin_norm_axis=-1,
                                              scope='LayerNorm')
    return output
...
def post_attention(h, attn_vec, d_model, n_head, d_head, dropout, is_training,
    ...
    if residual:
        output = tf.estimator.layers.layer_norm(attn_out + h, begin_norm_axis=-1,
                                              scope='LayerNorm')
    else:
        output = tf.estimator.layers.layer_norm(attn_out, begin_norm_axis=-1,
                                              scope='LayerNorm')
