...
def construct_scalar_host_call(
  ...
  def host_call_fn(global_step, *args):
    ...
    step = global_step[0]
    with tf.compat.v1.contrib.summary.create_file_writer(
        logdir=model_dir, filename_suffix=".host_call").as_default():
      with tf.compat.v1.contrib.summary.always_record_summaries():
        for i, name in enumerate(metric_names):
          if reduce_fn is None:
            scalar = args[i][0]
          else:
            scalar = reduce_fn(args[i])
          with tf.compat.v1.contrib.summary.record_summaries_every_n_global_steps(
              100, global_step=step):
            tf.compat.v1.contrib.summary.scalar(prefix + name, scalar, step=step)

        return tf.compat.v1.contrib.summary.all_summary_ops()
...
def get_qa_outputs(FLAGS, features, is_training):
  ...
  with @@#variable_scope("end_logits"):
    if is_training:
      ...
      start_index = tf.compat.v1.one_hot(start_positions, depth=seq_len, axis=-1,
      ...
      start_index = tf.compat.v1.one_hot(start_top_index,
                               depth=seq_len, axis=-1, dtype=tf.compat.v1.float32)
      start_features = tf.compat.v1.einsum("lbh,bkl->bkh", output, start_index)
      end_input = tf.compat.v1.tile(output[:, :, None],
      ...
      end_logits = tf.compat.v1.layers.dense(
          ...
          name="dense_0")
      end_logits = tf.compat.v1.contrib.layers.layer_norm(end_logits,
                                                begin_norm_axis=-1)
      end_logits = tf.compat.v1.layers.dense(
          end_logits,
          1,
          kernel_initializer=initializer,
          name="dense_1")
      end_logits = tf.compat.v1.reshape(end_logits, [seq_len, -1, FLAGS.start_n_top])
  ...
  with @@#variable_scope("answer_class"):
    ...
    cls_index = tf.compat.v1.one_hot(cls_index, seq_len, axis=-1, dtype=tf.compat.v1.float32)
    cls_feature = tf.compat.v1.einsum("lbh,bl->bh", output, cls_index)

    # get the representation of START
    start_p = tf.compat.v1.nn.softmax(start_logits_masked, axis=-1,
                            name="softmax_start")
...
def get_race_loss(FLAGS, features, is_training):
  ...
  return total_loss, per_example_loss, 