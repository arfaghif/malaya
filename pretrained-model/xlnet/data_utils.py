...
def _create_data(idx, input_paths):
    ...
    for input_path in input_paths:
        ...
        for line in tf.compat.v1.io.gfile.GFile(input_path):
...
def create_data(_):
    ...
    if FLAGS.task == 0:
        ...
        with tf.compat.v1.io.gfile.GFile(corpus_info_path, "w") as fp:
            json.dump(corpus_info, fp)

    # Interleavely split the work into FLAGS.num_task splits
    file_paths = sorted(tf.compat.v1.io.gfile.glob(FLAGS.input_glob))
    ...
    with tf.compat.v1.io.gfile.GFile(record_info_path, "w") as fp:
...
def create_tfrecords(save_dir, basename, data, bsz_per_host, seq_len,
    ...
    record_writer = tf.compat.v1.io.TFRecordWriter(save_path)
...
def _local_perm(inputs, targets, is_masked, perm_size, seq_len):
    ...
    non_func_tokens = tf.compat.v1.logical_not(tf.compat.v1.logical_or(
        tf.compat.v1.equal(inputs, SEP_ID),
        tf.compat.v1.equal(inputs, CLS_ID)))
    ...
    new_targets = tf.compat.v1.concat([inputs[0: 1], targets[: -1]],
                            axis=0)
...
def get_dataset(params, num_hosts, num_core_per_host, split, file_names,
    ...
    def parser(record):
        ...
        perm_mask_0 = tf.compat.v1.concat([perm_mask_0, tf.compat.v1.ones([reuse_len, non_reuse_len])],
                                axis=1)
        perm_mask_1 = tf.compat.v1.concat([tf.compat.v1.zeros([non_reuse_len, reuse_len]), perm_mask_1],
                                axis=1)
        ...
        if num_predict is not None:
            ...
            paddings = tf.compat.v1.zeros([pad_len, seq_len], dtype=target_mapping.dtype)
            target_mapping = tf.compat.v1.concat([target_mapping, paddings], axis=0)
            example["target_mapping"] = tf.compat.v1.reshape(target_mapping,
                                                   [num_predict, seq_len])
...
def get_input_fn(
    ...
    for idx, record_dir in enumerate(tfrecord_dirs):
        ...
        record_paths = sorted(tf.compat.v1.io.gfile.glob(record_glob))
        @@#logging.info("[%d] Num of record info path: %d",
        ...
        for record_info_path in record_paths:
            ...
            with tf.compat.v1.io.gfile.GFile(record_info_path, "r") as fp:
                info = json.load(fp)
                if num_passes is not None:
    ...
    @@#logging.info("Total number of batches: %d",
                    record_info["num_batch"])
    @@#logging.info("Total number of files: %d",
