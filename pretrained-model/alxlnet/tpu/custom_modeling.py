    ...
    with tf.compat.v1.variable_scope(scope, reuse = reuse):
        ...
        if use_tpu:
            ...
            if one_hot_idx.shape.ndims == 2:
                o = tf.compat.v1.einsum('in,nd->id', one_hot_idx, lookup_table)
                return (
                    tf.compat.v1.matmul(o, project_variable),
                    lookup_table,
                    project_variable,
                )
            else:
                o = tf.compat.v1.einsum('ibn,nd->ibd', one_hot_idx, lookup_table)
                o = tf.compat.v1.einsum('ibn,nd->ibd', o, project_variable)
...
def positionwise_ffn(
    ...
    with tf.compat.v1.variable_scope(scope, reuse = reuse):
        ...
        output = tf.compat.v1.estimator.layers.layer_norm(
